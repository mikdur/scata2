...
class FilteredCreateView(LoginRequiredMixin, CreateView):
    ...
    # Filter any field querysets subclassed from ScataModel to only
    # include owned objects
        ...
        for k in form.fields.keys():
            if (hasattr(form.fields[k], "queryset") and
               issubclass(form.fields[k].queryset.model, ScataModel)):
                form.fields[k].queryset = form.fields[k].queryset.\
                        filter(Q(owner=user, deleted=False) |
...
class DeleteToTrashView(LoginRequiredMixin, UserPassesTestMixin, DeleteView):
    def test_func(self):
        o = self.get_object()
...
# Override get_data with a function that returns
# a list of lists.
        ...
        response = \
            HttpResponse(content_type="text/csv",
                         headers={"Content-Disposition":
                                  ...
                                  format(fn=self.get_filename(context))},
                         **response_kwargs)
...
class ReferenceSetDetailView(OwnedDetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['errors'] = ScataRefsetErrorType.objects.all().\
            filter(refset=context['object'].pk)
...
class DataSetDetailView(OwnedDetailView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['errors'] = ScataErrorType.objects.all().\
            filter(dataset=context['object'].pk)
        context['tags'] = ScataTagStat.objects.all().\
            filter(dataset=context['object'].pk)
        context['discarded'] = context['object'].seq_total - \
            context['object'].seq_count
        return context
...
class DataSetTagsCSVView(CSVResponseMixin, DataSetDetailView):
    ...
    def get_filename(self, context):
        o = context['object']
        return urllib.parse.quote("dataset_{i}_{n}.csv".format(i=o.id,
                                                               n=o.name),
                                  safe="")
...
class DataSetTagsPCAJSONView(JSONResponseMixin, DataSetDetailView):
    ...
    def get_data(self, context):
        data = list(context['tags'].filter(in_pca=True).
